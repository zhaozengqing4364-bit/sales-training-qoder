...
- Test: `backend/tests/unit/test_newcomer_training_orchestration_journey_service.py`
...
- [ ] 写新字段解析、旧 payload 兼容和 Journey 完整投影失败测试。
- [ ] 运行聚焦 pytest，确认因字段缺失而失败。
- [ ] 添加有界长度、默认值和投影实现。
- [ ] 同步 TypeScript DTO，运行聚焦测试和类型检查。
...
- [ ] 写 ViewModel 新/旧合同、首页具体任务和唯一主操作失败测试。
- [ ] 运行 Vitest，确认 RED。
- [ ] 实现纯函数适配器与共享任务卡。
- [ ] 重排首页，移除巨大深色 Hero，将抽象阶段降为弱位置提示。
- [ ] 更新路径大纲展示阶段/模块 outcome 并保持折叠。
- [ ] 运行聚焦测试、axe/可访问性断言和移动布局检查。
...
- [ ] 写目标、步骤、通过标准和材料确认用户文案失败测试。
- [ ] 运行聚焦 Vitest，确认 RED。
- [ ] 在公共活动壳展示任务说明，并让执行器只负责能力交互。
- [ ] 将“已发布版本”等治理术语替换为学员语言，保留材料读取确认语义。
- [ ] 验证处理、失败、重试、完成和下一步状态。
...
- [ ] 写阶段 outcome、模块 outcome、活动目标/价值/步骤/标准 round-trip 失败测试。
- [ ] 运行 Vitest，确认 RED。
- [ ] 实现基础内容字段与可访问的字符串列表编辑器。
- [ ] 保持适用对象、前置依赖、完成策略和资源绑定在高级规则区。
- [ ] 新建节点写入兼容默认值，验证保存和重新加载不丢字段。
...
- [ ] 写两栏结构、按需预览、共享任务卡、草稿无需理由和低频操作可聚焦测试。
- [ ] 运行 Vitest，确认 RED。
- [ ] 将预览从常驻第三栏迁到大对话框，复用 `missionFromCandidate` 与任务卡。
- [ ] 默认只展开选中分支；行操作悬停/聚焦出现，键盘仍可用。
- [ ] 将保存/检查/发布移至顶部操作区；保存使用稳定审计说明，发布说明保持必填。
- [ ] 校验与历史进入次级折叠区，避免压缩主编辑区。
...
- [ ] 为默认三阶段和核心活动补充具体 outcome、目标、步骤与通过标准。
- [ ] 保证 seed 幂等且不覆盖管理员已发布内容。
- [ ] 更新 API 合同和 Trellis 可执行规范，记录字段限制与回退语义。
- [ ] 验证新环境 seed 和旧 revision 解析。
...
- [ ] 运行后端 newcomer 聚焦 pytest、Ruff、Mypy。
- [ ] 运行前端 newcomer Vitest、TypeScript、ESLint、production build。
- [ ] 运行桌面/移动学员路径和桌面管理编排 Playwright。
- [ ] 在实际运行页面检查主操作、移动首屏、编辑字段、真实预览和发布说明。
- [ ] 使用 CodeGraph affected/impact 选择补充回归并运行。
- [ ] 删除临时 `implementation-notes.md`，检查工作树，仅提交本任务文件。
