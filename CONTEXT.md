...
**配置边界**：V0.9 的训练路径顺序、PPT/录音任务、材料绑定、评分 Prompt、商务礼仪文章、小单元、题目、AI 补练教练启停、金字塔演讲任务和真实语音对练准入条件，都必须由后台配置和发布治理驱动；前端和服务端只能内置稳定模块类型、兼容 key、默认文案和安全兜底，不能把具体训练内容、材料、评分标准或关卡绑定写死。
