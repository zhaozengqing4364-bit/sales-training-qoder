...
> 与代码矛盾时，先更新本文件，再修代码。
> 2026-07-16 起，“新人训练路径”以 Foundation target 为权威；旧 V0.9、`sales_trainer`、Phase/Module 与 Realtime 首发描述仅是 Legacy 迁移语境。
...
**定义**：新人围绕一个已发布 **路径修订（PathRevision）** 完成基础训练的业务路径。PathRevision 直接包含有序 **训练阶段（Stage）** 与冻结 **训练活动定义（ActivityDefinition）**；不再以 Phase/Module 作为目标层级。

**首发活动**：只包含 `lesson`、`quiz`、`audio_assessment`、`ai_coach`、`assignment`。`assignment` 专指三段异步客户场景录音，每段冻结目标、材料、时限和评分/审核合同；不是通用文本或文件作业。

**正式结论**：`foundation_ready` 表示“基础训练达标”，只能由有对象范围的培训负责人基于版本化达标档案作出；它不表示真实客户场景胜任、岗位胜任或销售业绩预测。AI 只提供可追溯的初评、证据、推断或建议，不能授予正式结论。

**修订绑定**：Cohort 绑定一个已发布 PathRevision；Enrollment 创建后冻结该修订。发布新修订不会自动移动已有 Enrollment，迁移必须显式预览、确认、校验版本并审计。

**延期边界**：Realtime 客户语音对练不进入首发 ActivityDefinition、导航、seed、权限或验收。未来只能通过新 PathRevision 和独立、更高等级的 readiness 结论接入。
...
**目标命名空间**：学员使用 `/api/v1/newcomer-training`，管理使用 `/api/v1/admin/newcomer-training`。现有 `sales_trainer` 包和 `/sales-trainer` 路由是待按干净切换清单退役的 Legacy 实现名，不是目标领域语言或永久兼容面。

**活动而非模块**：Lesson 学习已发布 LearningUnit；Quiz 执行冻结 QuizRevision；Audio 完成录音评测；AI Coach 执行结构化训练卡与有限补练；Assignment 完成三段异步客户场景录音。具体产品、PPT、Demo、题目和评分标准都是修订化配置，不是新 Activity 类型。

**首发任务流**：分配冻结 Enrollment → 依次完成五类活动 → 形成不可变能力证据与达标档案 → 培训负责人复核 → 记录 `foundation_ready`、补练或补证据决定。Realtime 不在该流程中。

**Legacy V0.9 语境**：标准材料/PPT、商务礼仪和金字塔演讲等既有内容可作为迁移输入，但不能继续定义目标层级、活动联合、路由或达标口径。

**Legacy V0.9 全量闭环**：旧 V0.9 的“全量”强调配置、学习、提交、评分、补练、重练、复核、档案、准入、异常和审计闭环；该判断可作为迁移验收输入，但不覆盖 Foundation target 的对象、活动联合或正式结论。

**材料与提交边界**：标准 PPT、LearningUnit、题目和评分标准由管理员治理；新人提交的是答案、录音或三段场景录音，不应让新人把准备材料文件作为第一步负担。

**配置边界**：路径顺序、活动定义、材料绑定、Prompt/模型合同、题目、评分标准和补练策略都由有类型 working revision + ReleasePlan 发布治理驱动；前后端只内置五类稳定 Activity 能力、合同和安全兜底，不能写死具体内容或关卡绑定。
...
**关系**：一个冻结 Enrollment 对应一份当前 **训练达标档案**；档案由 ActivityOutcome、不可变能力证据、AI 初评和人工复核记录投影而成。

**禁止**：
- 不得把训练达标档案简化为模块完成进度或考试分数。
- 不得把未经确认的 AI 建议直接当作达标结论。

## 训练任务模板 (Training Task Template, Legacy)

**定义**：2026-07-16 之前用于描述后台训练关卡的 Legacy 术语。Foundation target 由 PathRevision 中的 Stage/ActivityDefinition 表达顺序、材料、提交、评分、能力映射和阻塞规则；新设计不得再创建并列的 TrainingTaskTemplate 权威。

**Legacy 类型**：材料学习、文章小测、录音评分、AI 补练、真实语音对练准入仅用于解释旧 V0.9 文档；目标首发联合以五类 ActivityDefinition 为准，Realtime 延期。

**关系**：旧训练任务实例迁移到对应 ActivityDefinition；上传、转写、评分、记录、复核或补练是应用命令/持久任务步骤，不成为管理员可编排的业务活动类型。

**禁止**：
- 不得让管理员直接编排上传、转写、评分、写记录等底层技术步骤。
- 不得把 PPT 讲解、商务礼仪、金字塔演讲等具体训练内容写死在页面或服务逻辑里；它们应由 ActivityDefinition 和发布修订驱动。

## 能力项 (Competency Item)

**定义**：新人训练中可被训练、评分、复核和判定达标状态的最小业务能力单元。

**来源**：首发能力项来自一套受治理的通用新人销售能力模型；ActivityDefinition 只能选择和引用这些能力项，AI 评分负责按冻结合同给出初评、证据和反馈，不负责临时发明新的能力项。

**关系**：一个 ActivityOutcome 可产生训练证据；冻结评分合同将证据映射到一个或多个 **能力项**；**训练达标档案** 汇总证据状态并等待人工决定。
...
**关系**：**金字塔演讲训练** 可配置为首发 Audio Activity 的具体训练内容；其评分方法可复用于 PPT 讲解、产品介绍和未来独立接入的真实语音对练。
...
**定义**：企业新人基础训练中一组稳定能力指标，用于跨活动汇总可追溯证据并支持人工判断是否达到 `foundation_ready`；它不证明真实岗位或客户场景胜任。

**第一版能力项**：表达清晰度、结构化讲解、产品理解、客户视角、需求识别、异议回应、商务礼仪与职业表达。

**关系**：一个 **通用新人销售能力模型** 包含 6-8 个 **能力项**；ActivityDefinition 引用其中若干项；AI 评分按冻结合同产生初评和证据，人工 ReviewDecision 形成正式结论。
