...
- [x] `operations` 的 `view_records=true`、`review_readiness=false`，POST review action 返回 403。
- [x] 平台管理员可复核任意部门 learner。
- [x] 培训负责人只能复核本部门 learner；跨部门返回 404。
- [x] `support`、`training_lead`、`training_manager` 按现有 manager allowlist 获得相同部门范围；allowlist 配置为空时 fail-closed。
- [x] 相同 actor、相同 idempotency key、相同请求重复提交返回同一 action，数据库只存在一条业务记录和一条审计日志。
- [x] 相同 idempotency key 携带不同请求内容返回 `[READINESS_IDEMPOTENCY_KEY_REUSED]` 409。
- [x] 陈旧 `expected_latest_review_action_id` 返回 `[READINESS_REVIEW_VERSION_CONFLICT]` 409，原决定不被覆盖。
- [x] 存在 legacy OperationLog 最新决定时，携带其 action/log ID 可以创建第一条专用 action；错误地携带 `null` 返回 409。
- [x] approve 仍要求 Dossier 为 `pending_review` 且存在有效证据。
- [x] approve 成功后 realtime gate 开放；require retraining/manual follow-up 后 gate 保持关闭。
- [x] 历史 OperationLog 动作仍可见，新动作以专用表为状态来源，且不会双重显示。
- [x] 前端首次操作进入明确确认状态，确认后才发送请求；提交中禁止重复点击。
- [x] 所有定向 backend/frontend tests、Ruff、ESLint、TypeScript type-check 通过。
- [x] 新测试进入 critical quality gate，migration upgrade 和安全回滚策略得到验证。
