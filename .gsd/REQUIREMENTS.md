...
- Validation: mapped; advanced by M002/S01 slice verification proving StepFun + classic sales runtime paths now emit the same five-dimension sales `score_update`/`stage_update`/`action_card` contract, the practice page preserves same-turn sales score refreshes instead of deduping by `overall_score + turn_count`, and both voice-mode entry points describe one shared sales rubric without changing the existing report-side three-rollup evidence contract. Further advanced by M002/S02 slice verification proving classic + StepFun now share one realtime-feedback arbiter, same-turn duplicate action cards are suppressed while stage/fuzzy/score context stays available, StepFun restore only persists minimal reconnect-safe pacing state to avoid replay bursts, and new final transcripts clear stale `actionCard` / `fuzzyDetections` so the practice panel keeps `action_card` as the only primary textual coach surface.
- Notes: S01 locked the realtime transport/UI contract to the current sales rubric. S02 retired the prompt-pileup risk by proving shared pacing, duplicate suppression, reconnect-safe replay control, and transcript-driven stale-hint clearing across backend + practice-page consumers. S03-S05 still need next-turn rule convergence, report/realtime consistency, and degraded-state visibility before R009 can move to validated.
