...
- Validation: Fresh M012 milestone close-out reran `backend/venv/bin/python -m pytest -c backend/pyproject.toml backend/tests/ -k password_reset -x -q` and it passed 6 selected tests covering token creation, local recovery visibility, non-enumeration, rate limiting, one-time use, and reset-password login authority.
...
- Primary owning slice: M012/S01
- Validation: Fresh M012 milestone close-out reran `npm --prefix web test -- --run dashboard` and it passed 28 tests, including assertions that the dashboard renders the real current-user name/email-prefix fallback and the dynamic `package.json` version rather than hardcoded placeholders.
...
| R029 | launchability | validated | M012/S01 | none | Fresh M012 milestone close-out reran `backend/venv/bin/python -m pytest -c backend/pyproject.toml backend/tests/ -k password_reset -x -q` and it passed 6 selected tests covering token creation, local recovery visibility, non-enumeration, rate limiting, one-time use, and reset-password login authority. |
| R030 | launchability | validated | M012/S01 | none | Fresh M012 milestone close-out reran `npm --prefix web test -- --run dashboard` and it passed 28 tests, including assertions that the dashboard renders the real current-user name/email-prefix fallback and the dynamic `package.json` version rather than hardcoded placeholders. |
