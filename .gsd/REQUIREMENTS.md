...
- Validation: mapped; S01 先锁定当前 admin Persona/knowledge → practice/runtime/report 入口链与 live knowledge status vocabulary，随后由 S02-S05 继续证明 Persona 压力 snapshot、多轮异议持续施压、claim truth contract 与 objection-heavy live UAT。
- Notes: 本要求的 done 必须锚定当前真实业务代码与用户价值链路：`web/src/app/admin/personas/[id]/page.tsx`、`web/src/app/admin/knowledge/[id]/page.tsx`、`backend/src/common/api/practice.py`、`backend/src/agent/services/persona_policy.py`、`backend/src/sales_bot/services/voice_runtime_policy.py`、`backend/src/sales_bot/services/voice_instruction_compiler.py`、当前 practice / knowledge-check / report / replay surfaces。Silence / Conda / `.env` / lockfile 等环境工件不计入 M003 验收，除非目标改为环境迁移。
...
| R010 | integration | active | M003/S01 | M003/S02, M003/S03, M003/S04, M003/S05 | mapped; S01 先锁定当前 admin Persona/knowledge → practice/runtime/report 入口链与 live knowledge status vocabulary，随后由 S02-S05 继续证明 Persona 压力 snapshot、多轮异议持续施压、claim truth contract 与 objection-heavy live UAT。 |
