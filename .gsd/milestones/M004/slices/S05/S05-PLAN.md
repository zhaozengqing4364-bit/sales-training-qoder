...
- [x] **T03: Captured the live PPT learner review loop on the shared report/replay routes and documented the current sibling-route replay entry plus degraded page-metadata behavior.** — Run one real PPT learning path on the current shared report/replay routes and capture the page-level evidence and degraded-proof behavior when relevant. Use the same acceptance artifact family as the sales path.
  - Estimate: 90m
  - Files: .gsd/milestones/M004/slices/S05/S05-UAT.md
  - Verify: Manual review — file exists and is non-empty
