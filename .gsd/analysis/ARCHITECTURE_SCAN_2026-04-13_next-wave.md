...
| `backend/src/prompt_templates/service.py` + `backend/src/prompt_templates/api/routes.py` | `/api/v1/prompt-templates*`; scenario/template lookup from evaluation and presentation interruption flows | prompt template admin routes, `evaluation/api.py`, evaluation/report services, presentation handlers | admin prompt governance UI/API, presentation interruption prompt fallback, legacy report/evaluation prompt resolution | **live governance + compat runtime helper** | Prompt templates are live as a control-plane/admin surface and still feed some runtime-adjacent helpers, but they are not the live sales StepFun instruction authority. |
...
| `backend/src/common/ai/llm_service.py::evaluate/generate_report` | only through evaluation services | `StagedEvaluationService`, `ComprehensiveReportService` | legacy staged evaluation JSON and detailed comprehensive-report text | **compat backend adapter** | Its current shipped role is to power the legacy evaluation/report stack. It is not called by the live StepFun instruction or canonical evidence pipeline directly. |

**T01 prompt source taxonomy / fake integration map（当前代码拥有的 inventory）**
- **live compiled contract**：`voice_runtime_policy.py` 会把 `persona_policy`、`customer_pressure`、`tool_policy` 交给 `voice_instruction_compiler.py`，产出 `policy["instructions"]` 与 `instruction_contract_hash`；这条 compiled artifact 才是 StepFun live/presentation realtime 真正消费的 prompt contract。
...
- **compat governance / runtime-adjacent helper**：`PromptTemplateService` 仍真实服务 admin prompt governance、legacy evaluation/report lookup、以及 presentation interruption template fallback；其中只有 presentation interruption resolver 会把拿到的 template 真正 render 成用户可见消息。
- **template bypass #1**：`evaluation/services/staged_evaluation.py::evaluate_stage` 会先 `get_template_for_scenario(...)`，随后把 `template_id + variables` 包进 `render_request` 交给 `LLMService.evaluate(...)`；但 `LLMService.evaluate(...)` 当前完全忽略 template text，只根据 `conversation/stage_name/stage_description` 重建一段硬编码评估 prompt，所以这是一条“取了 template 但没驱动模型调用”的真实假接入点。
- **template bypass #2**：`evaluation/services/comprehensive_report.py::_generate_detailed_feedback` 会先 lookup `report` 模板，但最终只把 `session_id/stage_count/overall_summary` 传给 `LLMService.generate_report(...)`；而后者同样重建硬编码报告 prompt，不消费 lookup 到的 template text，所以 report 侧也还是 compat adapter，不是 compiled prompt runtime。
- **code-owned proof surface**：后续如果有人要回答“哪个 prompt source 是 live、哪个只是 compat、哪里存在 fake integration”，先看 `prompt_templates/taxonomy.py::build_prompt_source_taxonomy()` 与 `common.ai.llm_service.LEGACY_PROMPT_ENTRYPOINTS`，不要重新凭 grep 猜测。
...
- 如果改的是 prompt governance UI / scenario template assignment，可以改 `PromptTemplateService`，但不要把它误写成 live sales runtime prompt authority。
