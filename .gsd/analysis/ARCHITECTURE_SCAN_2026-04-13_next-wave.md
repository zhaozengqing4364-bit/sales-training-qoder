...
- **T02 modular-monolith migration path（当前执行 contract）**：
  1. **Phase 1 — reader-first, still one monolith**：不先拆服务，也不先改 runtime 主链；优先在 `practice_sessions`、`manager_interventions`、`SessionEvidenceService` projection、`/admin/users/[id]`、`/admin/analytics`、manager-lite 上补 `organization_id` / `team_id` compatibility reader，让 report/replay/history/admin surfaces 可以按 `self/team/org/platform` scope 解释既有 evidence。
  2. **Phase 2 — authz seam before asset ownership**：先引入 `organization_member` + team assignment，把 org/team role 与 access scope 从 `users.role` 里拆出来；`get_current_admin_user()`、`require_role(...)`、`common.api.practice._can_read_session(...)` 进入 platform-role + membership-role + scope reader 的 compatibility path，而不是继续只认 global admin。
  3. **Phase 3 — rollout binding before org-owned content rows**：agent / persona / knowledge / prompt / voice-runtime / knowledge-config 继续保留 global template row，只增加 org rollout binding / visibility seam；这样 `voice_policy_snapshot_ref.runtime_binding`、report evidence、manager calibration 仍能说明“模板来源 + org 发布范围”，而不是因为提前复制资产把 truth line 打散。
  4. **Phase 4 — external integrations stay as adapters**：SSO、CRM、org sync 只负责写 membership / team / account metadata seam，不直接接管 session runtime、manager analytics、prompt runtime 或 knowledge truth；它们是 provisioning / metadata adapter，不是新的业务 authority。
  5. **Phase 5 — service split only with proof**：只有当 organization-scoped write path、membership sync、或 org-level analytics 因吞吐/隔离/合规要求出现独立扩缩容与发布节奏时，才值得从 modular monolith 拆服务；在那之前继续沿现有模块边界演进。
- **Migration ordering rule（固定）**：session/read-side scope reader → authz write path → asset rollout binding → external integration automation。任何新工作都不应跳过 compatibility reader，直接把当前 global row 改成 org-owned。
- **Compatibility-reader surfaces（T02 必补）**：
