...
- **M016/S01**：auth recovery backend 已从“基本可用”收口到正式 contract：PasswordResetService + PasswordResetToken + Alembic 026/027/028 成为权威 seam，single-active-token DB invariant 已落地，forgot/reset focused proof 覆盖成功、过期、重复使用、superseded token、same-IP rate limit，以及 hashed_password-first 登录兼容路径。
- **M016/S02**：高频 API 错误契约已在 audited prompt-template / presentation / auth 依赖面收口到统一 seam：route-local 4xx 走 `error_response(...)` 顶层 envelope，dependency/RBAC 失败走 `_raise_auth_http_error(...)` 的结构化 `detail={error,message}`，前端统一由 `normalizeApiErrorPayload` / `ApiRequestError` 消费，不再依赖 page-local 猜测；slice close-out fresh proof 为 backend 33/33 + web auth client 9/9 绿灯。
...
M016 正在执行中。S01、S02 已完成，当前焦点切到最后一块安全合同化工作：
- **S03**：补齐 admin 高风险接口的 RBAC 证明，以及敏感日志脱敏规则在高风险出口的落实。

避免把 M016 扩成新的登录/注册产品改造或全局异常框架重写；当前 auth / API / admin security 重点是合同化、RBAC 证明、日志脱敏与高风险出口治理。
...
- [ ] M016 — Auth / API / admin security contract hardening（S01-S02 complete, S03 pending）