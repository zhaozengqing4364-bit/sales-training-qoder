...
def _path_payload(
    ...
    def audio(
        ...
        example_transcripts = {
            "ppt": (
                "各位好，今天我会用三部分介绍我们能为客户创造的价值。首先，我们关注的是销售团队在新人上手慢、标准难统一、训练结果难追踪上的共同挑战。接下来，我会结合这份材料说明平台如何把学习、练习和评测连成一条可执行的训练路径。最后，我们会根据贵团队的目标确认最适合先落地的训练场景和下一步安排。"
            ),
            "demo": (
                "接下来我用一个真实工作场景演示产品。假设主管刚发布了一项新人产品讲解任务，学员进入后能先看到指定材料、评分重点和参考表达，完成录音后立即获得针对内容结构和表达方式的反馈。主管则能在后台查看完成进度与共性问题。这个流程的价值是让每次练习都有统一标准，也让改进有明确下一步。"
            ),
            "product-a": (
                "如果团队希望新人更快讲清产品 A，可以先从客户最常见的问题切入。产品 A 通过核心能力解决这一问题，并在标准业务流程中减少重复操作。对客户而言，直接收益是更快完成关键任务、降低协作成本。您目前更关注效率提升，还是过程标准化？我可以据此继续展开最相关的功能。"
            ),
            "product-b": (
                "产品 B 适合需要稳定管理复杂流程的团队。它把关键步骤、责任人和结果集中到同一个工作闭环中，减少信息遗漏，也让主管更早发现风险。实际使用时，我们建议先选择一个高频流程验证效果，再逐步扩展。贵团队现在最想先改善的是哪个流程？"
            ),
        }
        return {
            "activity_id": activity_id,
            ...
            "config": {
                ...
                "pass_score": 75,
                "max_attempts": 3,
                "example_transcript": example_transcripts.get(key),
            },
        }
