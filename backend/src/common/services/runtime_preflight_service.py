"""HTTP runtime preflight checks aligned with WebSocket connection gates."""
...
def _not_runnable(
    *,
    ...
    return RuntimePreflightResult(
        ...
        code=code,
        missing=missing or [],
        hint=hint or _PREFLIGHT_HINTS.get(code, "当前会话暂不可运行，请返回入口重试。"),
    )


def _runnable(*, runtime_type: str) -> RuntimePreflightResult:
    return RuntimePreflightResult(runnable=True, runtime_type=runtime_type)
...
class RuntimePreflightService:
    ...
    async def evaluate_session(self, session_id: str) -> RuntimePreflightResult | None:
    ...
    async def _check_sales(
        ...
        *,
        ...
        if await self._is_kb_lock_unbound(session):
            return _not_runnable(
                runtime_type=runtime_type,
    ...
    async def _check_examiner(self, session: PracticeSession) -> RuntimePreflightResult:
        ...
        for question_ref in question_refs:
            ...
            if not _asset_matches_ref(question, question_ref):
                return _not_runnable(
                    runtime_type=runtime_type,
                    code="EXAMINER_RUNTIME_SNAPSHOT_STALE",
                    missing=["question_item.version"],
                )

        return _runnable(runtime_type=runtime_type)

    async def _is_kb_lock_unbound(self, session: PracticeSession) -> bool:
