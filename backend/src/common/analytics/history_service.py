...
class HistoryService:
    ...
    def resolve_summary_issue_family(
        ...
        if not isinstance(issue_type, str) or not issue_type.strip():
            return None
        return cls.normalize_intervention_issue_family(issue_type)

    @classmethod
    def _build_pending_intervention_result(
        cls,
    ...
    @classmethod
    def _build_latest_completed_intervention_result(
        cls,
        intervention: ManagerIntervention,
        summary: HistorySessionSummary,
    ) -> ManagerInterventionResultSummary:
        normalized_target_family = cls.normalize_intervention_issue_family(
    ...
    def build_manager_intervention_results(
        ...
        for intervention in sorted(
            ...
            if latest_evaluable is not None:
                results.append(
                    cls._build_latest_completed_intervention_result(
                        intervention,
                        latest_evaluable,
                    )
                )
            ...
            if later_completed:
                ...
                continue
            results.append(cls._build_pending_intervention_result(intervention))
    ...
    async def get_manager_intervention_results(
        self,
        db: AsyncSession,
        user_id: uuid.UUID,
    ) -> Result[list[dict[str, Any]]]:
        try:
            interventions_result = await db.execute(
                ...
                .where(ManagerIntervention.user_id == str(user_id))
                .order_by(ManagerIntervention.created_at.desc())
            )
            ...
            self._log_history_query(
                ...
                user_id=str(user_id),
                summaries=summaries,
                filters={"intervention_count": len(interventions)},
                extra_fields={
                    "intervention_result_statuses": dict(
            ...
            return Result.fail(fallback="[ADMIN_INTERVENTION_RESULTS_FAILED]")
