...
practice_sessions_total = Counter(
    "practice_sessions_total", "Total practice sessions", ["scenario_type", "status"]
)
...
llm_requests_total = Counter(
    "llm_requests_total", "Total LLM requests", ["service", "status"]
)
...
llm_tokens_total = Counter(
    "llm_tokens_total", "Total LLM tokens consumed", ["service", "token_type"]
)

asr_requests_total = Counter("asr_requests_total", "Total ASR requests", ["status"])
...
def track_practice_session(
    ...
    practice_session_duration_seconds.labels(scenario_type=scenario_type).observe(
        duration
    )
    ...
    for score_type, score_value in scores.items():
        if score_value is not None:
            practice_scores.labels(
                scenario_type=scenario_type, score_type=score_type
            ).observe(score_value)
...
def track_llm_request(
    ...
    llm_request_duration_seconds.labels(service=service).observe(duration)
...
def track_websocket_message(
    ...
    websocket_message_duration_seconds.labels(
        scenario_type=scenario_type, message_type=message_type
    ).observe(duration)
