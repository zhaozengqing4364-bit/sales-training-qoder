...
class Settings:
    ...
    CORS_ORIGINS: list[str] = os.getenv(
        "CORS_ORIGINS",
        "http://localhost:3445,http://localhost:5173,http://localhost:3000"
    ).split(",")

    # Database
    DATABASE_URL: str = os.getenv(
        "DATABASE_URL",
        "sqlite+aiosqlite:///./ai_practice.db"
    )
    ...
    ASR_API_URL: str = os.getenv("ASR_API_URL", "wss://dashscope.aliyuncs.com/api-ws/v1/realtime")
    ...
    COST_WARNING_THRESHOLD: float = float(os.getenv("COST_WARNING_THRESHOLD", "0.8"))  # ¥0.8
    ...
    INTERRUPTION_DETECTION_TIMEOUT_MS: int = int(os.getenv("INTERRUPTION_DETECTION_TIMEOUT_MS", "100"))
    ...
    # Feature Flags
    ENABLE_BROWSER_ASR_FALLBACK: bool = os.getenv("ENABLE_BROWSER_ASR_FALLBACK", "true").lower() == "true"
