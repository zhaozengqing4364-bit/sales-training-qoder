...
class StepFunRealtimeHandler(
    ...
    async def _apply_lifecycle_action(
        ...
        try:
            async with AsyncSessionLocal() as db:
                ...
                try:
                    session_control = SessionControlAdapter(lifecycle_service)
    ...
    async def _commit_and_respond(self) -> None:
        ...
        self._audio_flow.commit_input_audio()
        self._has_uncommitted_audio = False
    ...
    async def _create_response(self, *, count_turn: bool = False) -> bool:
        ...
        turn_start = self._turn_coordinator.start_turn(str(self.current_request_id))
        ...
        self._turn_coordinator.on_model_response_start()
    ...
    async def _flush_active_response(self, response_done_event: dict) -> bool:
        ...
        self._active_response = None
        self._turn_coordinator.on_model_response_done()
        self._turn_coordinator.end_turn(str(response_state.request_id))
    ...
    def _build_stepfun_tools_from_policy(self) -> list[dict[str, Any]]:
        ...
        return self._tool_execution.build_tools_from_policy(self._effective_policy)

    def _enforce_stepfun_tool_guardrails(
        self, tools: list[dict[str, Any]]
    ) -> list[dict[str, Any]]:
        """Filter tool list using final effective policy guarantees."""
        return self._tool_execution.enforce_guardrails(tools, self._effective_policy)
