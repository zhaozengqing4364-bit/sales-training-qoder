"""Admin SQLAlchemy adapter for the neutral ConfigBundle lifecycle authority."""
...
class SqlAlchemyConfigLifecycleAdapter:
    """Persist lifecycle state while keeping SQLAlchemy out of the core service."""

    def __init__(
        self,
        db: AsyncSession,
        *,
        adapters: list[ConfigBundleAdapter],
    ) -> None:
        self._db = db
    ...
    def _adapter_for(self, bundle_key: str) -> ConfigBundleAdapter:
        adapter = next(
            (item for item in self._adapters if item.bundle_key == bundle_key),
            None,
        )
        if adapter is None:
    ...
    async def _sync_situation_pack_head_projection(
        ...
        *,
        ...
        if bundle_key != ROLEPLAY_SITUATION_PACKS_KEY:
            return None
        adapter = self._adapter_for(bundle_key)
        sync_head_projection = getattr(adapter, "sync_head_projection", None)
        if sync_head_projection is None:
            return None
        try:
            sync_result = await sync_head_projection(
                self._db,
                snapshot=snapshot,
                actor_id=actor_id,
            )
