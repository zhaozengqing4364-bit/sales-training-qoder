...
class TTSConfigSchema(BaseModel):
    ...
    volume: str = Field(
        default="+0%",
        description="Volume (e.g., +0%, +20%, -10%)"
    )
    pitch: str = Field(
        default="+0Hz",
        description="Pitch (e.g., +0Hz, +10Hz, -5Hz)"
    )
...
class BehaviorConfigSchema(BaseModel):
    ...
    challenge_frequency: float = Field(
        ...
        ge=0,
        le=1,
        description="Challenge frequency 0-1"
    )
    interruption_triggers: list[str] = Field(
        default_factory=list,
        description="Words that trigger interruption"
    )
    typical_questions: list[str] = Field(
        default_factory=list,
        description="Typical questions this persona asks"
    )
...
class CreateAgentRequest(AgentBase):
    ...
    capabilities_config: dict[str, Any] = Field(
        ...
        description="Capability module configurations"
    )
    default_knowledge_base_ids: list[str] = Field(
        default_factory=list,
        description="Default knowledge base IDs"
    )
...
class PersonaBase(BaseModel):
    ...
    description: str | None = Field(None, max_length=500, description="Persona description")
...
class CreatePersonaRequest(PersonaBase):
    ...
    knowledge_base_ids: list[str] = Field(
        default_factory=list,
        description="Persona-specific knowledge base IDs"
    )
    behavior_config: BehaviorConfigSchema = Field(
        default_factory=BehaviorConfigSchema,
        description="Behavior configuration"
    )
    scoring_weights: dict[str, float] | None = Field(
        None,
        description="Scoring weights to override Agent defaults"
    )
    tts_config: TTSConfigSchema | None = Field(
        None,
        description="TTS configuration (overrides system default)"
    )
...
class CreateAgentPersonaRequest(BaseModel):
    ...
    is_default: bool = Field(default=False, description="Whether this is the default persona")
    override_config: dict[str, Any] | None = Field(
        None,
        description="Configuration overrides for this association"
    )
