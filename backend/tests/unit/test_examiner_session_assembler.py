...
@pytest.mark.asyncio
async def test_assembler_falls_back_to_latest_published_examiner(
    test_db: AsyncSession,
    test_user: User,
) -> None:
    content = LearningContent(
    ...
    persisted = await test_db.get(PracticeSession, result.session.session_id)
    assert persisted is not None
    assert persisted.user_id == str(test_user.user_id)

    session_count = await test_db.execute(select(PracticeSession))
    assert len(list(session_count.scalars().all())) == 1
