...
def test_presentation_upload_path_rejects_traversal_absolute_nested_and_wrong_extension(
    tmp_path, filename
):
...
@pytest.mark.parametrize(
    "filename,extension", [("deck.ppt", ".ppt"), ("deck.pptx", ".pptx")]
)
def test_presentation_upload_path_uses_server_generated_name_under_root(
    tmp_path, filename, extension
):
