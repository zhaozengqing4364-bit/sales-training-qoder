...
async def test_get_document_chunks_falls_back_when_collection_unavailable(monkeypatch):
    ...
    processor = MagicMock()
    processor._read_document = AsyncMock(return_value="第一段内容。第二段内容。第三段内容。")
    processor._split_into_chunks.return_value = [
...
async def test_get_document_chunks_falls_back_when_vector_result_empty(monkeypatch):
    ...
    processor = MagicMock()
    processor._read_document = AsyncMock(return_value="A段。B段。")
    processor._split_into_chunks.return_value = [
