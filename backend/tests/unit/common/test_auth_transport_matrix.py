...
def test_auth_transport_matrix_marks_formal_and_compatibility_paths() -> None:
    ...
    assert AUTH_TRANSPORT_MATRIX["login_credentials"]["formal"] == ["user_hashed_password"]
...
def test_websocket_auth_prefers_header_then_query_compat_then_cookie() -> None:
    cookie_name = get_session_cookie_name()
    cookie_header = f"{cookie_name}=cookie-token"

    assert resolve_websocket_token(
