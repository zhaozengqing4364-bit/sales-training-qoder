    ...
    summary_mock = AsyncMock(
        return_value=Result.ok(
            SimpleNamespace(
                ...
                actionable_feedback="Anchor the next commitment.",
                score_confidence=80,
                score_persuasion=78,
                score_clarity=82,
            )
...
class TestSessionLifecycleTransitions:
    ...
    async def test_sales_lifecycle_state_transition_sequence(
        ...
        summary_mock.assert_awaited_once_with(uuid.UUID(str(session.session_id)))
    ...
    async def test_delete_compat_entry_is_idempotent_after_lifecycle_end_sales(
        self,
        async_client,
        auth_headers,
        db_session,
        ...
        scenario = Scenario(
            scenario_id=str(uuid.uuid4()),
            scenario_type="sales",
            name="integration_lifecycle_delete_sales",
            is_active=True,
        )
        session = PracticeSession(
            session_id=str(uuid.uuid4()),
        ...
        lifecycle_response = await async_client.post(
            f"/api/v1/practice/sessions/{session.session_id}/lifecycle",
            json={"action": "end"},
            headers=auth_headers,
        )
        assert lifecycle_response.status_code == 200
        assert lifecycle_response.json()["data"]["status"] == "scoring"
        ...
        assert delete_body["data"]["retry_entry"]["scenario_type"] == "sales"
