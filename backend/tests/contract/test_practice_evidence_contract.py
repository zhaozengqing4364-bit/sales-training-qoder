...
@pytest.mark.asyncio
async def test_report_and_replay_contract_expose_shared_canonical_kernel_and_compat_readers(
    async_client: AsyncClient,
    db_session: AsyncSession,
    owner: User,
    owner_headers: dict[str, str],
    ...
    scenario = Scenario(
        scenario_id=str(uuid.uuid4()),
        scenario_type="sales",
        name="canonical kernel contract scenario",
        is_active=True,
    )
    session = PracticeSession(
        session_id=str(uuid.uuid4()),
    ...
    compat = report_data["compatibility_readers"]
    ...
    assert compat["practice_session_rollup_fields_v1"] == {
        "logic_score": pytest.approx(report_data["logic_score"]),
        "accuracy_score": pytest.approx(report_data["accuracy_score"]),
