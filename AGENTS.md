...
- Do not batch unrelated fixes.
- Verify every change immediately.
- If you start temporary local verification services or other background processes for this repository, stop them before finishing the task. Do not leave crashed/stale `bg_shell` jobs behind, because they pollute later auto-mode health checks without reflecting a real product regression.
