...
export function ActivityShell({ detail, onRefresh }: ActivityRunnerProps) { const Runner = ACTIVITY_RUNNERS[detail.activity.activity_type]; return <main className="mx-auto min-h-screen max-w-3xl bg-slate-50 px-4 py-8"><Link href={`/newcomer-training/modules/${encodeURIComponent(detail.module_id)}`} className="text-sm text-slate-600">← 返回模块</Link><article className="mt-5 rounded-3xl bg-white p-6 shadow-sm md:p-8"><div className="border-b border-slate-100 pb-5"><p className="text-sm font-medium text-blue-700">训练活动</p><h1 className="mt-1 text-2xl font-semibold text-slate-900">{detail.activity.title}</h1>{detail.activity.description && <p className="mt-2 text-slate-500">{detail.activity.description}</p>}</div><div className="mt-6">{detail.activity.locked ? <div role="alert" className="rounded-xl bg-amber-50 p-4 text-amber-900">{detail.activity.lock_reason ?? "请先完成前置活动。"}</div> : <Runner detail={detail} onRefresh={onRefresh} />}</div></article></main>; }
