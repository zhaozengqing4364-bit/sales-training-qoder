    ...
    return <article className="flex flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        <div className="order-1 border-b border-slate-100 px-5 py-5 sm:px-7">
            <div className="flex flex-wrap items-center gap-2 text-sm text-slate-500">
                ...
                {mission.moduleLabel ? <><span aria-hidden="true">·</span><span>{mission.moduleLabel}</span></> : null}
            </div>
        ...
        <div className={cn(
            "grid gap-6 px-5 py-6 sm:px-7",
            compact ? "order-2" : "order-3 lg:order-2 lg:grid-cols-[minmax(0,1.25fr)_minmax(260px,0.75fr)]",
        )}>
            <div>
                ...
                <ol className={cn("mt-3 grid gap-3", !compact && "sm:grid-cols-3")}>
        ...
        <div className={cn(
            "flex flex-col gap-3 bg-slate-50/70 px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-7",
            compact ? "order-3 border-t border-slate-100" : "order-2 border-b border-slate-100 lg:order-3 lg:border-b-0 lg:border-t",
        )}>
