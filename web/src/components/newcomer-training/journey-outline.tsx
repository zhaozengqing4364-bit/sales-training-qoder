        ...
        {phases.map((phase) => {
            ...
            return <div key={phase.phase_id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                <button type="button" aria-expanded={open} aria-label={`${phase.title} ${stateLabel}`} onClick={() => setExpanded((current) => ({ ...current, [phase.phase_id]: !open }))} className="flex w-full items-center gap-3 px-5 py-4 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-blue-500">
                    ...
                    <span className="min-w-0 flex-1"><span className="block font-semibold text-slate-900">{phase.title}</span><span className="text-sm text-slate-500">{phase.locked ? phase.lock_reason : progressLabel(phase.completed_count, phase.total_required)}</span></span>
                ...
                {open && <div className="border-t border-slate-100 px-5 py-3"><ol className="space-y-3">{phase.modules.map((moduleConfig) => <li key={moduleConfig.module_id}><Link href={`/newcomer-training/modules/${encodeURIComponent(moduleConfig.module_id)}`} className="flex items-center justify-between gap-3 rounded-xl px-3 py-2 hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"><span><span className="block text-sm font-medium text-slate-800">{moduleConfig.title}</span><span className="text-xs text-slate-500">{moduleConfig.locked ? moduleConfig.lock_reason : `${moduleConfig.completed_count}/${moduleConfig.total_required} 个必修活动`}</span></span><span className="text-xs text-slate-500">{moduleConfig.activities.find((item) => !item.completed)?.locked ? "等待解锁" : activityStatusLabel(moduleConfig.activities.find((item) => !item.completed) ?? moduleConfig.activities.at(-1)!)}</span></Link></li>)}</ol></div>}
            </div>;
        })}
    </section>;
}
