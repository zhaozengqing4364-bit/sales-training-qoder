        ...
        {phases.map((phase) => {
            ...
            return <div key={phase.phase_id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                <button type="button" aria-expanded={open} aria-label={`${phase.title} ${stateLabel}`} onClick={() => setExpanded((current) => ({ ...current, [phase.phase_id]: !open }))} className="flex w-full items-center gap-3 px-5 py-4 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-blue-500">
                    ...
                    <span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs text-slate-600">{stateLabel}</span><span data-chevron-motion aria-hidden="true" className={`text-slate-400 transition-transform duration-[var(--duration-popover)] ease-[var(--ease-out)] motion-reduce:transform-none ${open ? "rotate-180" : ""}`}><ChevronDown className="h-4 w-4" /></span>
                </button>
                <AnimatePresence initial={false}>
                    {open ? <motion.div
                        key={`${phase.phase_id}-content`}
                    ...
                    ><ol className="space-y-2">{phase.modules.map((moduleConfig) => <li key={moduleConfig.module_id}><Link href={`/newcomer-training/modules/${encodeURIComponent(moduleConfig.module_id)}`} className="flex items-center justify-between gap-3 rounded-xl px-3 py-2.5 hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"><span className="min-w-0"><span className="block text-sm font-medium text-slate-800">{moduleConfig.outcome || moduleConfig.title}</span><span className="mt-0.5 block text-xs text-slate-500">{moduleConfig.outcome ? `${moduleConfig.title} · ` : ""}{moduleConfig.locked ? moduleConfig.lock_reason : `${moduleConfig.completed_count}/${moduleConfig.total_required} 个必修活动`}</span></span><span className="shrink-0 text-xs text-slate-500">{moduleConfig.activities.find((item) => !item.completed)?.locked ? "等待解锁" : activityStatusLabel(moduleConfig.activities.find((item) => !item.completed) ?? moduleConfig.activities.at(-1)!)}</span></Link></li>)}</ol></motion.div> : null}
                </AnimatePresence>
            </div>;
        })}
    </section>;
