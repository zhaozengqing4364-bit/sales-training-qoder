...
>(({ className, ...props }, ref) => (
    <DialogPrimitive.Overlay
        ref={ref}
        className={cn(
            "motion-dialog-overlay fixed inset-0 z-50 bg-slate-900/10 backdrop-blur-[2px]",
            className
        )}
        {...props}
    />
    ...
    return (
        <DialogPortal>
            ...
            <DialogPrimitive.Content
                ...
                className={cn(
                    "motion-dialog-content fixed left-[50%] top-[50%] z-50 grid w-full max-w-lg gap-4 border border-white/60 bg-white/70 backdrop-blur-2xl p-6 shadow-[0_20px_50px_rgba(0,0,0,0.1)] sm:rounded-[2rem]",
                    className
                )}
                {...props}
            >
                {children}
                <DialogPrimitive.Close className="absolute right-4 top-4 rounded-full p-2 opacity-70 ring-offset-white transition-[color,background-color,opacity] duration-[var(--duration-press)] ease-[var(--ease-out)] hover:bg-slate-100 hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-slate-100 data-[state=open]:text-slate-500">
                    <X className="h-4 w-4" />
