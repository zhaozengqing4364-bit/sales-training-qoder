...
export function PathEditor({ initialModel, onSave, onValidate, onPublish, resources: initialResources = EMPTY_RESOURCES, onResourcesNeeded }: PathEditorProps) {
    ...
    return <div className="space-y-4">
        <header className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="flex flex-wrap items-start justify-between gap-4"><div><div className="flex flex-wrap items-center gap-2"><h1 className="text-2xl font-semibold text-slate-950">新人训练路径</h1><span role="status" data-save-state={dirty ? "dirty" : "saved"} className={cn("inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium transition-[color,background-color] duration-[var(--duration-press)] ease-[var(--ease-out)]", dirty ? "bg-amber-100 text-amber-800" : "bg-emerald-100 text-emerald-800")}>{!dirty ? <CheckCircle2 aria-hidden="true" className="h-3.5 w-3.5" /> : null}{dirty ? "有未保存修改" : "草稿已保存"}</span></div><p className="mt-1 text-sm text-slate-500">配置学员要完成的阶段、模块和任务。当前 {draft.phases.length} 个阶段、{moduleCount} 个模块。</p></div>
