...
describe("api.testBank contract", () => {
    ...
    it("lists questions with no filters", async () => {
        ...
        expect(result.items[0].scoring_dimensions).toEqual(["逻辑性", "说服力"]);
        expect(result.items[0].scoring_criteria).toEqual({ logic_weight: 0.6, persuasion_weight: 0.4 });
        expect(result.total).toBe(1);
    });
    ...
    it("creates a question", async () => {
        fetchMock.mockResolvedValue({
            ...
            json: async () => ({
                ...
                data: {
                    ...
                    status: "draft",
                    tags: [],
                    scoring_dimensions: ["基础知识"],
                    scoring_criteria: { knowledge: 1.0 },
                    safety_flagged: false,
                    department: null,
                    version: 1,
                    created_at: "2026-05-15T00:00:00Z",
        ...
        expect(fetchMock).toHaveBeenCalledWith(
            ...
            expect.objectContaining({
                method: "POST",
                body: expect.stringContaining('"title":"新题目"'),
            }),
        );
    });
    ...
    it("handles publish gate error", async () => {
        fetchMock.mockResolvedValue({
            ...
            json: async () => ({
                detail: {
                    error: "[QUESTION_ITEM_PUBLISH_GATE_FAILED]",
                    message: "缺少参考答案",
                },
            }),
        });
